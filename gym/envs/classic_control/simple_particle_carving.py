...
class SimplerParticleCarving(gym.Env):
    def __init__(self):
        ...
        self.obs_dim = self.numCells * self.numCells + 4
        ...
        self.pos_high = np.ones(self.act_dim) * self.world_size_view / 2
    ...
    def _step(self, action):
        ...
        reward = -0.01 * np.sum(self.particleSim.positions[self.particleSim.positions[:, 0] >= 0][:, 0])
        # print(reward)
    ...
    def _get_obs(self):
        obs = np.concatenate([self.curr_pos, self.curr_vel, self.density_map.flatten() / 5]).ravel()
        return obs
    ...
    def do_simulation(self, act, frameskip):
        ...
        self.curr_vel += tau * self.dt
        self.curr_pos = np.clip(self.curr_pos + self.curr_vel * self.dt, self.pos_low, self.pos_high)

        x_0 = np.array([self.curr_pos[0], self.curr_pos[1] + self.half_length])
        x_1 = np.array([self.curr_pos[0], self.curr_pos[1] - self.half_length])
    ...
    def _reset(self):
        ...
        self.curr_pos = np.array([1, 0]) + np.random.uniform(-0.25, 0.25, 2)
        self.curr_vel = np.array([0, 0]) + np.random.uniform(-0.1, 0.1, 2)
    ...
    def _render(self, mode='human', close=False):
        ...
        if self.curr_act is not None:
            ...
            x_1 = np.array([self.curr_pos[0], self.curr_pos[1] + self.half_length])
            x_2 = np.array([self.curr_pos[0], self.curr_pos[1] - self.half_length])
            norm_dir = np.array([-1, 0])
