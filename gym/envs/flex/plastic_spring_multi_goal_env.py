...
class PlasticSpringMultiGoalReshapingEnv(flex_env.FlexEnv):
    ...
    def _step(self, action):
        ...
        prev_group1_parts = np.reshape(prev_group1_parts,(self.numInstances,int(prev_group1_parts.shape[0]/self.numInstances),prev_group1_parts.shape[1]))

        prev_group2_parts = prev_state[:,4::][self.partIdxGoal2]
        prev_group2_parts = np.reshape(prev_group2_parts,(self.numInstances,int(prev_group2_parts.shape[0]/self.numInstances),prev_group2_parts.shape[1]))
        ...
        prev_distance_group1 = 0.1 * np.sum(np.linalg.norm(prev_group1_parts - expanded_group1_centers, axis=2) , axis=1)
        prev_distance_group2 = 0.1 * np.sum(np.linalg.norm(prev_group2_parts - expanded_group2_centers, axis=2) , axis=1)
        ...
        curr_distance_group1 = 0.1 * np.sum(np.linalg.norm(curr_group1_parts - expanded_group1_centers, axis=2) , axis=1)
        curr_distance_group2 = 0.1 * np.sum(np.linalg.norm(curr_group2_parts - expanded_group2_centers, axis=2) , axis=1)

        obs = self._get_obs()

        rewards = (prev_distance_group1 - curr_distance_group1) + (prev_distance_group2-curr_distance_group2)
