...
class PlasticSpreadingRotXYHeightEnv(flex_env.FlexEnv):
    def __init__(self):
        ...
        action_bound = np.array([[-5, -5, -5, -np.pi / 2,-np.pi / 2], [5, 5, 5, np.pi / 2,np.pi / 2]])
    ...
    def _step(self, action):
        ...
        for i in range(self.numInstances):
            ...
            if(outliers.shape[0]>0):
                curr_outlier_dist[i] = np.sum(np.clip(outliers-self.innerRatio*self.mapHalfExtent,0,abs(outliers-self.innerRatio*self.mapHalfExtent)))
